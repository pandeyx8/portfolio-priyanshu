        ...
        >
            ...
            >
                <Image
                    ...
                    height={2000}
                    alt='plogo'
                    className={`${isMobile ? 'w-12 h-12' : 'w-14 h-14 sm:w-16 sm:h-16'} object-contain object-center rounded-full`}
                />
            </motion.div>
            <motion.span
    ...
    return (
        ...
        >
            ...
            {SECTION_ITEMS.map((item, index) => (
                ...
                >
                    ...
                    >
                        <motion.span
                            className={`relative ${isMobile ? 'px-4 py-3 text-base' : 'px-4 py-2'} text-gray-300 hover:text-white transition-colors`}
                            whileHover={{ scale: isMobile ? 1 : 1.05 }}
                            whileTap={{ scale: 0.95 }}
                        >
                            {item.label}
        ...
        {NAV_ITEMS.map((item, index) => (
            <motion.div
                ...
                initial={isMobile ? { opacity: 0, x: -20 } : { opacity: 0, y: -20 }}
                animate={{ opacity: 1, x: 0, y: 0 }}
                transition={{ delay: isMobile ? (SECTION_ITEMS.length + index) * 0.1 : 0.1 * (SECTION_ITEMS.length + index) }}
                onClick={onLinkClick}
            >
